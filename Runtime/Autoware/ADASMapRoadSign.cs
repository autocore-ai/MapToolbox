...
{
    class ADASMapRoadSign : ADASMapElement<ADASMapRoadSign>
    {
        ...
        public Type RoadSignType { get; set; }
        ...
        public override string ToString() => $"{ID},{Vector.ID},{Pole.ID},{(int)RoadSignType},{LinkLane.ID}";
        const string file = "roadsign.csv";
            ...
            if (data != null && data.Count() > 0)
            {
                foreach (var item in data.Split(','))
                {
                    new ADASMapRoadSign
                    {
                        ...
                        RoadSignType = (Type)int.Parse(item[3]),
